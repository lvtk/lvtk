...
LVTK includes backends and supporting API for
...
LVTK only provides a portable mechanism to load the Vulkan library and get the functions used to load the rest of the Vulkan API.
...
but LVTK provides a portable function, :func:`createSurface`,
...
LVTK does not provide API that uses ``vulkan.hpp`` to avoid the onerous dependency,
